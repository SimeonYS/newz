...
class NewzPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('newz.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `newz`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM newz WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO newz (date, title, link, content)"
